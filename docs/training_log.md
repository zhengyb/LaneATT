...
- 数据集： 按新的规则，重新生成数据集。Valid和test数据集由于随机分割会变动。
- Valid性能:
...
## 17. 2025 April 29 AM2, 使用更新标签后的数据集进行初步训练. 对pred采用3次样条插值.
- 数据集： 同上
- anchors文件： 同上。
- epochs: 20. T_max: 20* 10814
- 训练参数修改：
- 训练时长： 预期4个小时左右；
- 最优性能(Valid数据集): ![Metrics](./img/tusimple_metrics_plot-20250429AM2.png) 
...
- 转换openlane的test数据集，评估特殊场景下的性能；
- 更多epochs的训练--DONE
## 16. 增加LLAMAS数据比例，提升高速场景性能？？